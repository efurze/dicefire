...
module.exports = {
	...
	recordWin:function(uid) {
		loadLogger();
		logger.log("recordWin", uid, logger.LEVEL.DEBUG, logger.CHANNEL.SUBMIT);
		return module.exports.getAI(uid)
			.then(function(str) {
				...
				logger.log("Error recording win", err, logger.LEVEL.ERROR, logger.CHANNEL.SUBMIT);
			});
	},

	recordLoss: function(uid) {
		loadLogger();
		logger.log("recordLoss", uid, logger.LEVEL.DEBUG, logger.CHANNEL.SUBMIT);
		return module.exports.getAI(uid)
			.then(function(str) {
				var info = JSON.parse(str);
