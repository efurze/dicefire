...
module.exports = {
	...
	saveGameInfo: function(gameId, info) {
		getLogger().ASSERT(info instanceof Gameinfo);
		return redisClient.setAsync(KEYS.GAME_INFO(gameId), info.toString());
	},
	...
	recordWin:function(uid) {
		...
		return module.exports.getAI(uid)
			.then(function(str) {
				...
				logger.log("Error recording win", err, logger.LEVEL.ERROR, logger.CHANNEL.DEFAULT);
			});
	},
	...
	recordLoss: function(uid) {
		...
		return module.exports.getAI(uid)
			.then(function(str) {
				...
				logger.log("Error recording loss", err, logger.LEVEL.ERROR, logger.CHANNEL.DEFAULT);
			});
	},
